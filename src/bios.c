...
struct TermContext* termCtxCreate(int width_in_tiles, int height_in_tiles) {
  ...
  *term = (struct TermContext){0};
  ...
  int width = width_in_tiles * ASCII_TILE_SIZE * ASCII_SCALE;
  int height = height_in_tiles * ASCII_TILE_SIZE * ASCII_SCALE;
  ...
  gpuBufferCreate(term->gpu.allocator, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
        MAX_LAYERS * tile_buffer_size * sizeof(struct GpuPackedTile),
        &term->tile_indices);

  gpuBufferCreate(term->gpu.allocator, VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT,
