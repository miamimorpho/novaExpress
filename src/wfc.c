 ...
 * Original Author: Krystian Samp (samp.krystian at gmail.com)
...
int wfc_img_save(struct wfc_image *image, const char *filename) {
  ...
  if (len < 4) goto UNKNOWNFORMAT;

  strcpy(extension, filename + len - 4);
  for (int i = 0; i < 4; i++) extension[i] = tolower(extension[i]);
...
// Return NULL on error
static struct wfc_image *wfc__img_expand(struct wfc_image *image, int xexp,
                                         int yexp) {
  ...
  for (int y = 0; y < exp_image->height; y++) {
    memcpy(
        &exp_image->data[y * exp_image->width * image->component_cnt],
...
static int wfc__img_cmpoverlap(struct wfc_image *a, struct wfc_image *b,
                               enum wfc__direction direction) {
...
static struct wfc_image *wfc__img_flip_vertically(struct wfc_image *image) {
  ...
  for (int y = 0; y < image->height / 2; y++) {
    unsigned char *src =
        &(image->data[y * image->width * image->component_cnt]);
    unsigned char *dst = &(copy->data[(image->height - 1 - y) * image->width *
                                      image->component_cnt]);
  ...
  for (int y = 0; y < image->height; y++) {
    for (int x = 0; x < image->width; x++) {
      ...
      memcpy(components,
             ...
             image->component_cnt);
      if (n == 1) {
        memcpy(&(copy->data[x * copy->width * copy->component_cnt +
                            (copy->width - y - 1) * copy->component_cnt]),
               components, image->component_cnt);
      } else if (n == 2) {
        memcpy(&(copy->data[(copy->height - y - 1) * copy->width *
                                copy->component_cnt +
...
static void wfc_destroy_cells(int *cells) { free(cells); }
...
static int *wfc_cells(struct wfc *wfc) {
  ...
  for (int i = 0; i < wfc->cell_cnt; i++) cells[i] = wfc->cells[i].tiles[0];
...
static struct wfc_image *wfc__create_tile_image(struct wfc_image *image, int x,
...
// Return 0 on error
static int wfc__add_overlapping_images(struct wfc__tile *tiles,
                                       struct wfc_image *image, int xcnt,
                                       int ycnt, int tile_width,
                                       int tile_height) {
  ...
  for (int y = 0; y < ycnt; y++)
    for (int x = 0; x < xcnt; x++) {
      ...
      if (tile->image == NULL) goto CLEANUP;
    }

  return 1;

CLEANUP:
  for (int i = 0; i < tile_cnt; i++) wfc_img_destroy(tiles[i].image);
...
static int wfc__add_flipped_images(struct wfc__tile *tiles, int tile_idx,
                                   int flip_direction) {
  for (int i = 0; i < tile_idx; i++) {
    ...
    if (dst->image == NULL) goto CLEANUP;
  }
...
CLEANUP:
  ...
  for (int i = 0; i < tile_idx; i++) wfc_img_destroy(tiles[tile_idx + i].image);
...
CLEANUP:
  ...
  for (int i = tile_idx; i < tile_idx * 4; i++) wfc_img_destroy(tiles[i].image);
...
// Return 0 on error, non-0 on success
static int wfc__remove_duplicate_tiles(struct wfc__tile **tiles,
                                       int *tile_cnt) {
  int unique_cnt = 1;
...
static void wfc__destroy_props(struct wfc__prop *props) { free(props); }
...
static struct wfc__cell *wfc__create_cells(int cell_cnt, int tile_cnt) {
  ...
  if (cells == NULL) goto CLEANUP;

  for (int i = 0; i < cell_cnt; i++) cells[i].tiles = NULL;
  ...
  for (int i = 1; i < cell_cnt; i++) {
    ...
    if (cells[i].tiles == NULL) goto CLEANUP;
  }
...
static void wfc__add_prop(struct wfc *wfc, int src_cell_idx, int dst_cell_idx,
                          enum wfc__direction direction) {
...
static int wfc__tile_enabled(struct wfc *wfc, int tile_idx, int cell_idx,
...
static int wfc__is_prop_pending(struct wfc *wfc, int cell_idx,
                                enum wfc__direction d) {
...
static int wfc__propagate_prop(struct wfc *wfc, struct wfc__prop *p) {
  ...
  // Go through all destination tiles and check whether they are enabled by the
  // source cell
...
static void wfc__init_cells(struct wfc *wfc) {
  int sum_freqs = 0.0;
  for (int i = 0; i < wfc->tile_cnt; i++) sum_freqs += wfc->tiles[i].freq;
...
static void wfc__compute_allowed_tiles(int *allowed_tiles[4],
                                       struct wfc__tile *tiles, int tile_cnt) {
  for (int d = 0; d < 4; d++) {
    for (int i = 0; i < tile_cnt; i++) {
...
// Return NULL on error
static struct wfc__tile *wfc__create_tiles_overlapping(
    struct wfc_image *image, int tile_width, int tile_height, int expand_image,
    int xflip_tiles, int yflip_tiles, int rotate_tiles, int *tile_cnt) {
  ...
  *tile_cnt = xcnt * ycnt;
  ...
  if (xflip_tiles) {
    if (!wfc__add_flipped_images(tiles, base_tile_cnt, 0)) goto CLEANUP;
    base_tile_cnt *= 2;
  }

  if (!(xflip_tiles && rotate_tiles) && yflip_tiles) {
    if (!wfc__add_flipped_images(tiles, base_tile_cnt, 1)) goto CLEANUP;
    base_tile_cnt *= 2;
  }

  if (rotate_tiles) {
    if (!wfc__add_rotated_images(tiles, base_tile_cnt)) goto CLEANUP;
    base_tile_cnt *= 4;
  }

  if (!wfc__remove_duplicate_tiles(&tiles, tile_cnt)) goto CLEANUP;
...
// Return NULL on error
struct wfc *wfc_overlapping(int output_width, int output_height,
                            struct wfc_image *image, int tile_width,
                            int tile_height, int expand_input, int xflip_tiles,
                            int yflip_tiles, int rotate_tiles) {
  struct wfc *wfc = malloc(sizeof(*wfc));
  if (wfc == NULL) goto CLEANUP;
  ...
  if (wfc->tiles == NULL) goto CLEANUP;
  ...
  if (wfc->cells == NULL) goto CLEANUP;

  wfc->props = wfc__create_props(wfc->cell_cnt);
  if (wfc->props == NULL) goto CLEANUP;
